...
def load_data(orders_path: Path, 
              items_path: Path, customers_path: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Carrega os dados de pedidos, itens e clientes a partir
    de arquivos csv.

    Args:
    orders_path: Caminho para os arquivos csv
        items_path: Caminho para os arquivos csv
        customers_path: Caminho para os arquivos csv

    Return:
        Tuple com os DataFrames de pedidos, itens e clientes
    """
...
def save_data(dataset: pd.DataFrame, output_path: Path) -> None:
    ...
    try:
        ...
        logger.info(f"Dataset salvo com sucesso em {output_path}")
...
def aggregate_items(items: pd.DataFrame) -> pd.DataFrame:
    ...
    # Retorna as cinco primeiras linhas da tabela agregada.
    return items_agg
